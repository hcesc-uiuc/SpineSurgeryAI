...
enum SupportedMetric: String, CaseIterable {
    ...
    var hkType: HKSampleType? {
            ...
            switch self {
            case .heartRate:        return getQuantity(.heartRate)
            case .steps:            return getQuantity(.stepCount)
            case .hrv:              return getQuantity(.heartRateVariabilitySDNN)
            case .oxygen:           return getQuantity(.oxygenSaturation)
            case .calories:         return getQuantity(.activeEnergyBurned)
            case .walkingSpeed:     return getQuantity(.walkingSpeed)
            case .walkingAsymmetry: return getQuantity(.walkingAsymmetryPercentage)
            case .walkingSteadiness: return getQuantity(.appleWalkingSteadiness)
            case .sleep:            return HKCategoryType.categoryType(forIdentifier: .sleepAnalysis)
            case .restingHeartRate: return getQuantity(.restingHeartRate)
            case .bodyTemperature:  return getQuantity(.bodyTemperature)
            case .exerciseTime:     return getQuantity(.appleExerciseTime)
            case .standTime:        return getQuantity(.appleStandTime)
            }
        }
...
class HealthKitManager: ObservableObject {
    ...
    func requestPermissions() {
        let allTypes = Set(SupportedMetric.allCases.compactMap { $0.hkType })
        healthStore.requestAuthorization(toShare: nil, read: allTypes) { success, _ in
            if success {
                for metric in SupportedMetric.allCases {
                    if let sampleType = metric.hkType {
    ...
    private func setupBackgroundObserver(for type: HKSampleType) {
        ...
        let query = HKObserverQuery(sampleType: type, predicate: nil) { [weak self] _, completion, error in
            ...
            guard let metric = SupportedMetric.allCases.first(where: { $0.hkType?.identifier == type.identifier }) else {
                completion(); return
            }
            
            if metric == .sleep {
                Task {
                    await self?.fetchSleepData(daysBack: 1, isHistorical: false) { completion() }
                }
            } else {
                Task {
                    await self?.fetchDeepData(for: metric, daysBack: 1, isHistorical: false) { points in
                        DispatchQueue.main.async {
                            guard let self = self else { return }
                            self.trialData = (points + self.trialData)
                                .prefix(2000)
        ...
        let query = HKAnchoredObjectQuery(type: type, predicate: predicate, anchor: anchorToUse, limit: HKObjectQueryNoLimit) { [weak self] _, samples, deleted, newAnchor, error in
            ...
            let points = samples.map { sample in
                RawDataPoint(
                    ...
                    deviceName: sample.device?.name, deviceModel:sample.device?.model, softwareVer:sample.device?.softwareVersion, metadata: sample.metadata
                )
            }
