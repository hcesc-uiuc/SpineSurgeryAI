...
# Batch size for CSV reading (number of rows per batch)
...
def analyze_uploaded_data(kind: str, content_bytes: bytes) -> Dict[str, Any]:
    ...
    # Try CSV first (batch reading for memory efficiency)
    try:
        print("[checker] analyze_uploaded_data: trying CSV parse (batch mode)", flush=True)
        ...
        if reader.fieldnames and "timestamp" in reader.fieldnames:
            ...
            for row in reader:
                ...
                if len(batch) >= CSV_BATCH_SIZE:
                    timestamps_ms.extend(batch)
                    batch_count += 1
                    print(f"[checker] Processed batch {batch_count}, total timestamps so far: {len(timestamps_ms)}", flush=True)
                    batch = []
            ...
            print(f"[checker] CSV parse done, {batch_count} batch(es), timestamps={len(timestamps_ms)}", flush=True)
